...
def pull_data():
    ...
    mycursor.execute("SELECT Owner, Name, latitude, longitude, address FROM naemergen")
    myresult_emaus = mycursor.fetchall()
    for x in myresult_emaus:
        names.append(x[0])
        phones.append(x[1])
        em_longs.append(x[2])
        em_lats.append(x[3])
        addresses.append(x[4])

    return lats, longs, brights, names, comps, phones, em_longs, em_lats, addresses
...
def nearest_emergency(user_lat, user_long, rad, names, comps, phones, em_longs, em_lats, adds):
    ...
    return dist[indx], phones[indx], names[indx], adds[indx]
...
def receive_data():
    ...
    [em_dist, em_phone, em_name, em_address] = nearest_emergency(mylat, mylong, myrad, data[3], data[4], data[5], data[6], data[7], data[8])
    
    print(f"There are {numfires} fires near you. There are {numintense} high risk fires. Closest emergency center, {em_name} is {round(em_dist)}km away on {em_address} and their number is {em_phone}")
    ...
    _ = client.messages \
                .create(
                    body=f"There are {numfires} fires near you. There are {numintense} high risk fires. Closest emergency center, {em_name} is {round(em_dist)}km away on {em_address} and their number is {em_phone}",
                    from_='+12018907595',
                    to='+13188346207'
               )
    return res
